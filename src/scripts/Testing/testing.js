  ...
  tooltip
    ...
    .html(
      `<div class='row'><b>Date: ${timeFormat(
        parseDate(d.date)
      )}</b></br> Daily PCR tests: ${numberFormat(
        d.daily_pcr
      )} </br> 7-day avg. PCR tests: ${numberFormat(d.avg_pcr)}</div>`
    )
}
...
function ready(data) {
  ...
  data=data.filter(d=> +d.avg_pcr < 70000)
  data=data.filter(d=> +d.daily_pcr > 0)

  /// map each column////
  const dates = data.map(d => d.datetime)
  const dailyTests = data.map(d => +d.avg_pcr+10000)

  /// update Scales////
  xScale.domain(d3.extent(dates))
  yScale.domain([0, d3.max(dailyTests)])

  svg
    .append('g')
    .selectAll('rect')
    .data(data.filter(d=> +d.daily_pcr < 100000))
    .enter()
    .append('rect')
    .attr('class', 'daily-cases-bars')
    .attr('x', d => xScale(d.datetime))
    .attr('y', d => yScale(+d.daily_pcr))
    .attr('width', width / dates.length - 0.2)
    .attr('height', d => height - yScale(+d.daily_pcr))
    .attr('fill', 'orange')
  ...
  svg
    .append('text')
    .attr('class', 'headline')
    .text('Daily PCR tests in Kentucky')
    .attr('font-size', '18')
    .attr('font-weight', 'bold')
  ...
  function render() {
    ...
    svg
      ...
      .attr('height', d => newHeight - yScale(+d.daily_pcr))
    ...
    svg
      .selectAll('.avgLabel')
      .attr('x', xScale(parseDate('2020-04-01')))
      .attr('y', yScale(8000))
