...
function renderData(data) {
  ...
  const pctCases = data.map(d => +d.pctCases)
  // console.log(data)

  /// update Scales////
  xScale.domain(d3.extent(dates))
  yScale.domain([0, d3.max(pctCases) + 1000])
  ...
  svg
    .append('text')
    .attr('class', 'headline')
    .text('Total cases by race per 100k in Kentucky')
    .attr('font-size', '18')
    .attr('font-weight', 'bold')
    // .attr('fill', 'orange')
  ...
  function render() {
    ...
    line.x(d => xScale(d.datetime)).y(d => yScale(+d.pctCases))
    ...
    svg
      .selectAll('.raceLabel')
      .attr('y', function(d) {
        return yScale(+d.values[5].pctCases)
      })
      .attr('x', function(d) {
        return newWidth / 2
      })
    ...
    svg
      ...
      .text(function(d) {
        if (newWidth < 400) {
          return 'Total cases by race per 100k in KY'
        } else {
          return 'Total cases by race per 100k in Kentucky'
        }
      })
      .attr('font-size', '16')
