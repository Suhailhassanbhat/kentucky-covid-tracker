  ...
  tooltip
    ...
    .html(
      `<div class='row'><b>Date: ${timeFormat(
        parseDate(d.date)
      )}</b></br> Daily deaths: ${numberFormat(
        d.actual_deaths
...
function ready (data) {
  ...
  const daily = data.map(d => +d.actual_deaths)

  /// update Scales////
  xScale.domain(d3.extent(dates))
  yScale.domain([0, d3.max(daily) + 10])
  ...
  svg
    ...
    .attr('y', d => yScale(+d.actual_deaths))
    .attr('width', width / dates.length - 0.2)
    .attr('height', d => height - yScale(+d.actual_deaths))
  ...
  svg
    ...
    .attr(
      ...
      d3
        ...
        .y(d => yScale(+d.avg_actual_deaths))
    )
  ...
  svg
    .append('text')
    .attr('class', 'headline')
    .text('Deaths by day in Kentucky')
    .attr('font-size', '18')
    .attr('font-weight', 'bold')
    .attr('text-anchor', 'start')
  ...
  function render() {
    ...
    svg
      ...
      .attr('height', d => newHeight - yScale(+d.actual_deaths))
    ...
    svg
      .selectAll('.label')
      .attr('x', xScale(parseDate('2020-12-16')))
      .attr('y', yScale(62))
