  ...
  tooltip
    ...
    .html(
      `<div class='row'><b>Date: ${timeFormat(
        parseDate(d.date)
      )}</b></br> Daily cases per 100k: ${numberFormat(
        d.cases_per_100k
      )} </br> 7-day average cases per 100k: ${numberFormat(
        d.avg_cases_per_100k
      )}</div>`
    )
}
...
function ready(data) {
  ...
  const dailyavgCasesPerCapita = data.map(
    d => +numberFormat(+d.cases_per_100k)
  )

  /// update Scales////
  xScale.domain(d3.extent(dates))
  yScale.domain([0, d3.max(dailyavgCasesPerCapita) + 5])
  ...
  svg
    ...
    .attr('y', d => yScale(+d.cases_per_100k))
    .attr('width', width / dates.length - 0.2)
    .attr('height', d => height - yScale(+d.cases_per_100k))
    .attr('fill', 'orange')
  ...
  svg
    ...
    .attr(
      ...
      d3
        ...
        .y(d => yScale(+d.avg_cases_per_100k))
    )
  ...
  svg
    .append('text')
    .attr('class', 'headline')
    .text('Cases per 100,000 people in Kentucky')
    .attr('font-size', '18')
    .attr('font-weight', 'bold')
    // .attr('fill', 'orange')
  ...
  function render() {
    ...
    svg
      ...
      .attr('height', d => newHeight - yScale(+d.cases_per_100k))
    ...
    svg
      ...
      .text(function(d) {
        if (newWidth < 400) {
          return 'Cases per 100k people in Kentucky'
        } else {
          return 'Cases per 100,000 people in Kentucky'
        }
      })
